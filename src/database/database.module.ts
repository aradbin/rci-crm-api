...
const providers = [
  ...modelProviders,
  {
    ...
    useFactory: async () => {
      const knex = Knex({
        client: 'pg',
        connection: `postgres://${process.env.POSTGRES_USER}:${process.env.POSTGRES_PASSWORD}@${process.env.POSTGRES_HOST}:${process.env.POSTGRES_PORT}/${process.env.POSTGRES_DB}?sslmode=${process.env.POSTGRES_SSL}`,
      });
